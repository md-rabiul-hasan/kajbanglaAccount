                ...
                <?php 
                    ...
                    foreach($menus['parent_menu'] as $parent_menu){
                        if(count($parent_menu['submenu']) > 0){ // multiple menu
                            ?>
                                <li class="<?php if( ($parent_menu['menu_id'] == $current_page['parent_id']) || ($parent_menu['menu_id'] == $current_page['root_parent_id']) ){ echo "active"; } ?>">
                                    ...
                                    foreach($parent_menu['submenu'] as  $submenu){
                                        if(count($submenu['submenu_submenu']) > 0){ // multiple sumenut
                                            ?>
                                                <li class="<?php if( ($submenu['menu_id'] == $current_page['parent_id'])){ echo "active"; } ?>">
                                                    <a href="#"><?php echo $submenu['menu_name']; ?><span class="fa arrow"></span></a>
                                                    <ul class="nav nav-third-level">
                                                        <?php 
                                                            foreach($submenu['submenu_submenu'] as $submenu_submenu){
                                                                ?>
                                                                    <li class="<?php if($submenu_submenu['menu_id'] == $current_page['id']){ echo "active"; } ?>" ><a href = " <?php echo $submenu_submenu['link']; ?>"> <?php echo $submenu_submenu['menu_name']; ?></a></li>
                                                                <?php
                                                            }
                                                        ?>                                                           
                                                    </ul>
                                            ...
                                            ?>
                                                <li class="<?php if($submenu['menu_id'] == $current_page['id']){ echo "active"; } ?>"><a href=" <?php echo $submenu['link']; ?>"> <?php echo $submenu['menu_name']; ?></a></li>
                                            <?php
                                        }
